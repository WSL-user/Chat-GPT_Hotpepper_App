...
function RestaurantSearch(props) {
    ...
    useEffect(() => {
        if (searchResult && searchResult.length > 0) {
        ...
        } else if (chatGPTtext==="Loading" && searchResult && searchResult.length==0) {
            ...
            if (counter === LIMIT) {
                setChatGPTText(`We are sorry to inform that no results are available from current query : "${query}"`);
                setCounter(0);
            }
        }
    }, [searchResult])
